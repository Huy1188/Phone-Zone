...
let handleAdminLogin = async (req, res) => {
  ...
  try {
    ...
    if (!user)
      return fail(res, 400, "Email này chưa được đăng ký trong hệ thống!");

    const passOk = bcrypt.compareSync(password, user.password);
    if (!passOk) return fail(res, 400, "Mật khẩu không chính xác!");

    if (user.role_id !== 1)
      return fail(res, 403, "Tài khoản này không có quyền Quản trị!");
  ...
  try {
    ...
    if (data.selected_default_id) {
      ...
      await db.Address.update(
        { is_default: true },
        { where: { address_id: data.selected_default_id } }
      );
    }
...
let handleCreateBrand = async (req, res) => {
  ...
  try {
    ...
    if (data.slug && data.slug.trim() !== "")
      finalSlug = slugify(data.slug, { lower: true, locale: "vi" });
...
let getOrderDetail = async (req, res) => {
  try {
    ...
    return ok(res, { order, orderDetails }, "Chi tiết đơn hàng");
  } catch (e) {
    console.log(e);
    return fail(res, 500, "Lỗi: " + e);
  }
